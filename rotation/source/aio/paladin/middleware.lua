...
rotation_registry:register_middleware({
	...
	matches = function(context)
		...
		if context.hp > threshold then
			return false
		end
		if context.forbearance_active then
			return false
		end
		return true
	end,
...
rotation_registry:register_middleware({
	...
	execute = function(icon, context)
		local HealthStoneObject =
			DetermineUsableObject(PLAYER_UNIT, true, nil, true, nil, A.HealthstoneMaster, A.HealthstoneMajor)
...
rotation_registry:register_middleware({
	...
	matches = function(context)
		if not context.settings.use_healing_potion then
			return false
		end
		if not context.in_combat then
			return false
		end
		if context.combat_time < 2 then
			return false
		end
...
rotation_registry:register_middleware({
	...
	matches = function(context)
		if not context.settings.use_mana_potion then
			return false
		end
		if not context.in_combat then
			return false
...
rotation_registry:register_middleware({
	...
	matches = function(context)
		if not context.settings.use_dark_rune then
			return false
		end
		if not context.in_combat then
			return false
...
rotation_registry:register_middleware({
	...
	matches = function(context)
		if not context.settings.use_seal_of_wisdom_low_mana then
			return false
		end
		if not context.in_combat then
			return false
		...
		if context.seal_wisdom_active then
			return false
		end
...
rotation_registry:register_middleware({
	...
	matches = function(context)
		if not context.settings.use_cleanse then
			return false
		end
		if context.is_mounted then
			return false
		...
		if not hasPoison and not hasDisease and not hasMagic then
			return false
		end
		return true
	end,
...
rotation_registry:register_middleware({
	...
	matches = function(context)
		...
		if not context.settings.use_hammer_of_justice then
			return false
		end
...
rotation_registry:register_middleware({
	...
	matches = function(context)
		...
		if (Unit(PLAYER_UNIT):HasBuffs(Constants.AURA_BUFF_IDS) or 0) > 0 then
			return false
		end
		return true
	end,
...
rotation_registry:register_middleware({
	...
	matches = function(context)
		...
		if (Unit(PLAYER_UNIT):HasBuffs(Constants.BLESSING_BUFF_IDS) or 0) > 0 then
			return false
		end
		return true
	end,
